...
const SocialIcon = ({ path, label }: { path: string; label: string }) => (
  <Link
    href="#"
    aria-label={label}
    className="w-8 h-8 flex items-center justify-center text-white/70 hover:text-[--color-ember] transition-colors"
  >
...
const TopBar = () => {
  ...
  return (
    <div className="hidden md:flex w-full bg-[#0D1815] border-b border-[#1C2924] items-stretch justify-between text-sm">
      {/* Left: chat icon + marquee */}
      <div className="flex items-stretch">
        <div className="w-14 bg-[--color-ember] flex items-center justify-center">
          <svg viewBox="0 0 24 24" fill="#000" className="w-5 h-5">
            <path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-6 12H6v-2h8v2zm4-4H6V8h12v2z" />
          </svg>
        </div>
        <div className="hidden lg:flex items-center px-6 text-white/80 font-medium tracking-wide uppercase text-xs">
          Full Stack + AI Engineer · React Native · Node.js · LLM / STS / TTS
        </div>
      </div>
      ...
      <div className="flex items-stretch">
        ...
        <button
          ...
          className="bg-[--color-ember] text-black font-bold uppercase tracking-wider text-xs px-7 flex items-center gap-2 hover:bg-[--color-spark] transition-colors"
        >
